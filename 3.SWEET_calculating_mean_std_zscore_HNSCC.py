...
# parser.add_argument("-p", type=str, default="./example/samples.txt", help="A path to 'samples of interest' file")
...
# parser.add_argument("-s", type=str, default="./example", help="A path to the output file(s)")
...
def sweet3(file_p, save) :
    ...
    with open(save+'/mean_std.txt', mode='w') as wline:
    ...
    for p in patlist:
        ...
        if not os.path.exists(file):
            print(f"{file} not found")
            sys.exit()
        with open(f"{save}/{p}.txt", mode='r') as rline, open(f"{save}/{p}_zscore.txt", mode='w') as wline:
            _ = rline.readline()
