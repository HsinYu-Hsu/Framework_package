...
def gene_set_network(DEG_list_result_df, criterion, output_dir):
    if criterion=='fdrq':
        legend_figure = './example/graph_files/legend_fdrq.png'
        ...
        legend_figure = './example/graph_files/legend_rawp.png'
        enrich_DEGlist_df = DEG_list_result_df[DEG_list_result_df['P-value']<0.5]
    ...
    graph_files_database = './example/graph_files/'
    ...
    #write html file
    fileo_html = open(os.path.join(output_dir,'{sample}_{pvalue}_rwr{alpha}_subnetwork_pathway_graph.html'),'w')
    ###html backbone
    filef1_html_path = './example/graph_code_fragment/html_f1.txt'
    filef1_html = open(filef1_html_path)
    html_f1 = filef1_html.read()
    filef1_html.close()
    
    filef2_html_path = './example/graph_code_fragment/html_f2.txt'
    filef2_html = open(filef2_html_path)
    html_f2 = filef2_html.read()
    filef2_html.close()
    
    filef3_html_path = './example/graph_code_fragment/html_f3.txt'
    ...
    ###js backbone
    filef1_js_path = './example/graph_code_fragment/js_f1.txt'
    filef1_js = open(filef1_js_path)
    js_f1 = filef1_js.read()
    filef1_js.close()
    
    filef2_js_path = './example/graph_code_fragment/js_f2.txt'
    filef2_js = open(filef2_js_path)
    js_f2 = filef2_js.read()
    filef2_js.close()

    filet1_cyto_path = './example/graph_code_fragment/cyto_t1.txt'
    filet1_cyto = open(filet1_cyto_path)
    cyto_t1 = filet1_cyto.read()
    filet1_cyto.close()
    
    filet2_cyto_path = './example/graph_code_fragment/cyto_t2.txt'
...
### ens -> name
data_e = {}
with open("./example_input/geneid_ens2name.txt",'r')as f:
    _g = f.readline()
    for line in f:
        g_e, ent = line.strip('\n').split(',')     
        g_e = [(g_e)]
...
with open(f'{input_dir}/KEGG_api_347_hsa_pathway_20220518_v102_gene_symbol.txt','r') as f :
    for line in f :
